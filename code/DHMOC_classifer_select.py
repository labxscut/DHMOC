...
# Function to intelligently wrap labels based on spaces
def wrap_labels_logically(labels):
    
    """Wrap labels logically based on spaces"""
    wrapped = []
    for label in labels:
        # If the label is a numeric type, convert to string
        ...
        if len(words) <= 2:  # If there are only 1-2 words, don't wrap
            wrapped_label = label
        else:  # If more than 2 words, wrap every 2 words onto a new line
            wrapped_label = ''
...
# Function to get colors for dataset based on hierarchical clustering
def get_dataset_colors(Z, cell_unique_types):
    ...
    color_palette = [
        ...
        '#FFB6C1',  # Light Pink
        '#DDA0DD'   # Medium Violet-Red
    ]
    # Ensure there are enough colors
    if len(cell_unique_types) > len(color_palette):
        # If the number of cell types exceeds predefined colors, cycle through the colors
    ...
    # Define the function to get color for a given index
    def get_color(k):
        # If it's a leaf node
...
class LazyClassifier:
    ...
    start = time.time()
    try:
        # Create model instance (only once)
        ...
        # Use the predict method to get prediction results consistently
        # 2. If probability predictions are needed
        ...
        # Combine true labels of the training and test sets
        true_labels = np.concatenate([y_train, y_test])
        

        le = LabelEncoder()
        le.fit(cell_unique_types)  # Train the encoder with the fixed label order
        ...
        # 4. Use the same order when creating the confusion matrix
        conf_mat = confusion_matrix(
            true_labels, 
            final_predictions,
            labels=np.arange(len(cell_unique_types))  # [0,1,2,3]
        )
        
        # Normalize each row (divide by the maximum value of the row)
        ...
        fig = plt.figure(figsize=(15, 9), dpi=1200)  # Increase width ratio

        dendrogram(
            Z,
            labels=wrap_labels_logically(cell_unique_types),
            leaf_rotation=0,
            leaf_font_size=13,
            truncate_mode='level',
            p=5,
        ...
        classifier_part = name.lower().replace(' ', '_')
        ...
        # Use tight_layout but keep sufficient margins
        plt.tight_layout(rect=[0, 0.1, 1, 0.95])

        # Save the image
        save_dir = '/work2/wlp/DHMOC/DHMOC/DHMOC_new/result/new_result'
        dendrogram_filename = os.path.join(save_dir, f'{dataset_part}_{classifier_part}_dendrogram.pdf')
        plt.savefig(dendrogram_filename, format='pdf', bbox_inches='tight', dpi=1200)
        plt.close()
        ...
        # # Save as .mat file
        # mat_filename = os.path.join(save_dir, f'{dataset_part}_{classifier_part}_roc_data.mat')
        # sio.savemat(mat_filename, mat_data)
        # print(f"ROC data saved to: {mat_filename}")

        # Plot the graph

                plt.figure(figsize=(15, 6))

                # 1. ROC curve of training set (left)
                plt.subplot(1, 2, 1)
                for i, label in enumerate(cell_unique_types_list):
                ...
                precision = precision_score(y_test, y_pred, average='weighted')  # add
                recall = recall_score(y_test, y_pred, average='weighted') 
                print("评价测度计算完毕，接下来计算ROC AUC")
                try:
                ...
                F1.append(f1)
                Precision.append(precision)  # add
                Recall.append(recall)        # add
