...
def wrap_labels_logically(labels):
    ...
    for label in labels:
        ...
        if len(words) <= 2:  # If only 1-2 words, no wrapping
            wrapped_label = label
...
def get_dataset_colors(Z, cell_unique_types):
    ...
    def get_color(k):
        ...
        # If it's an internal node, use the color of the left child
    ...
    def fit(self, dataset_name,cell_unique_types,X_train, X_test, y_train, y_test):
        ...
        for name, model in tqdm(self.classifiers):
            ...
            try:
                ...
                # Always use the predict method to get predictions
                ...
                # Combine true labels from train and test sets
                true_labels = np.concatenate([y_train, y_test])

                le = LabelEncoder()
                le.fit(cell_unique_types)  # Train the encoder with labels in fixed order
                ...
                # 4. Create confusion matrix with the same order
                ...
                # Normalize each row (divide by the row's maximum value)
                ...
                # Note: scipy's linkage function requires a compressed distance matrix format
                # Since the matrix is symmetric, we only need the upper triangle part
                ...
                fig = plt.figure(figsize=(15, 9), dpi=1200)  # Increase width proportion
                ...
                # Use tight_layout but leave enough margins
                ...
                # Prepare data to save as MAT file
                ...
                # mat_data = {
                #     'Z': Z,
                #     'cell_types': np.array(cell_unique_types, dtype=object),
                #     'dataset_name': dataset_name
                # }
                
                # Save as MAT file
                # dataset_part = dataset_name.lower().replace('-', '_')
                # classifier_part = name.lower().replace(' ', '_')  # name is classifier name
                # mat_filename  = os.path.join(save_dir, f'{dataset_part}_{classifier_part}_dendrogram.mat')
                # sio.savemat(mat_filename, mat_data)
                # print(f"Graphic saved to: {mat_filename}")
                # plt.close()

                # Get training and testing probabilities
                train_probs = pipe.predict_proba(X_train)
                test_probs = pipe.predict_proba(X_test)

                # If cell_unique_types is numpy array, convert to list
                ...
                # Compute and store ROC data for train and test sets
                ...
                # Convert to numpy arrays to ensure compatibility with MATLAB
